...
@zedub.on(events.NewMessage(pattern="/tof"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (6799580948)
    if user.id in zed_dev:
        await event.reply(f"أهلًا وسهلًا بك مطوري توفي  - @BDTHON")


@zedub.on(events.NewMessage(pattern="/rio"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (5943382651)
    if user.id in zed_dev:
        await event.reply(f"أهلًا بك عزيزي ريو - @BDTHON")
