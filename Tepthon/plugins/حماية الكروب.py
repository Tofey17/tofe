...
@Tepthon.ar_cmd(pattern="قفل ?(.*)", groups_only=True, require_admin=True)
async def _(event):
...
@Tepthon.ar_cmd(pattern="فتح ?(.*)", groups_only=True, require_admin=True)
async def _(event):
    input_str = event.pattern_match.group(1)
    chat_id = event.chat_id
    if not event.is_group:
...
@Tepthon.ar_cmd(pattern="الاعدادات$", groups_only=True)
async def _(event):
    res = ""
...
@Tepthon.ar_cmd(incoming=True, forword=None)
async def check_incoming_messages(event):
    ...
    if is_locked(chat_id, "rtl") and (
        ...
        if Tepthon == razan or await is_admin(event, Tepthon) or Tepthon in devs:
            return
        else:
            try:
                await event.delete()
...
@Tepthon.ar_cmd(pattern=f"البوتات ?(.*)")
