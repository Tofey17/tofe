...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**✦ جـاري فحـص تنصيبـك لـ 𝗕𝗗𝗧𝗛𝗢𝗡 ⌬ . .**")
    end = datetime.now()
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** بـوت بــدثــون 𝗕𝗗𝗧𝗛𝗢𝗡 يعمـل بنجـاح ☑️ .. **"
...
**{Z_EMOJI} قنـاة الـسـورس :** [اضغـط هنـا](https://t.me/BDTHON)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "⎆╿"
    zed_caption = "** بـوت بــدثــون 𝗕𝗗𝗧𝗛𝗢𝗡 يعمـل بنجـاح ☑️ .. **\n"
