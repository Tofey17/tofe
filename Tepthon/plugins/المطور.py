...
from Tepthon import StartTime, zedub, JEPVERSION
...
@zedub.on(events.NewMessage(pattern=r'\.event', outgoing=True))
async def my_event_handler(event):
    if event.is_reply:
        ...
        if replied_message:
            ...
            await zedub.send_file(event.chat_id, "event_info.txt")
...
async def amireallyalive(event):
    ...
    CAT_IMG = " https://graph.org/file/adb45d0b9b4691dfbeffc.jpg "
    if CAT_IMG:
        CAT = [x for x in CAT_IMG.split()]
        A_IMG = list(CAT)
        PIC = random.choice(A_IMG)
        cat_caption = f"مطورين بدثون\n"
        cat_caption += f"✛━━━━━━━━━━━━━✛\n"
        cat_caption += f"- المطور  : @B_8_K\n"
        cat_caption += f"- المطور  : @R_i_OO\n"
        cat_caption += f"✛━━━━━━━━━━━━━✛\n"
