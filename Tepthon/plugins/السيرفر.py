...
async def variable(event):  # sourcery no-metrics
    ...
    if cmd == "جلب":
        ...
        for i in configs:
            if variable in i:
                _, val = i.split("= ")
                return await cat.edit("𓆩 𝗕𝗗𝗧𝗛𝗢𝗡 - 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار** `{variable} = {val}`")
        await cat.edit(
            "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗕𝗗𝗧𝗛𝗢𝗡 - 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار :** -> {variable} **غيـر موجود**❌"
        )
    elif cmd == "ضع":
        ...
        for i in configs:
            if variable in i:
                ...
                string += f"{i}"
        if match:
            await cat.edit(f"**- تم تغيـر** `{variable}` **:**\n **- المتغيـر :** `{value}` \n**- يتم الان اعـادة تشغيـل بــدثــون  يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        else:
            string += f"    {variable} = {value}\n"
            await cat.edit(
                f"**- تم إضـافـة** `{variable}` **:**\n **- المضـاف اليـه :** `{value}` \n**- يتم الان اعـادة تشغيـل بـوت بــدثــون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**"
            )
...
async def _(event):
    ...
    zed = await edit_or_reply(
        ...
        f"**⌔∮ يتـم الآن إعادة تشغيـل بـوت بــدثــون فـي السيـرفـر قـد يستغـرق الامـر 2-3 دقيقـة ▬▭ ...**",
    )
    if cmd == "كلين":
        for file in exts:
